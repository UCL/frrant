...
CKEDITOR.editorConfig = function (config) {
	...
	config.extraPlugins = "vinculumOn,vinculumOff,underdotOn,underdotOff,floating-tools";

	if (enableFootnotes) {
		config.extraPlugins += ",footnotes";
		config.toolbar[6].items.push("Footnotes");
	}
	...
	if (enableMentions || enableApparatusCriticus) {
		...
		if (enableApparatusCriticus) {
			config.mentions.push({
				...
				// outputTemplate: "<span class='mention' data-target={target} data-id={id}>{list_display}</span>"
				outputTemplate: `<span class='mention'
									data-denotation-char='#'
									data-target='{target}'
									data-id='{id}'
