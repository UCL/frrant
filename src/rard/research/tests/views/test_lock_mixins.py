...
from rard.research.views import AntiquarianDetailView, AntiquarianWorkCreateView
...
class TestCanLockMixin(TestCase):
    def test_post_creates_lock(self):
        ...
        antiquarian = Antiquarian.objects.create()
        url = reverse("antiquarian:detail", kwargs={"pk": antiquarian.pk})
        data = {
            "name": "name",
            "subtitle": "subtitle",
        ...
        # call the view
        AntiquarianDetailView.as_view()(request, pk=antiquarian.pk)
    ...
    def test_post_fails_if_locked(self):
        ...
        # call the view
        response = AntiquarianDetailView.as_view()(request, pk=antiquarian.pk)
    ...
    def test_post_lock_until(self):
        ...
        data = {"name": "name", "subtitle": "subtitle", "days": 5}
        request = RequestFactory().post(url, data=data)
        request.user = UserFactory.create()
