...
class RemoveFragmentLinkView(
    ...
    """
    Just a POST endpoint for deleting a Fragment link.

    Two modes of operation: if the form has ``antiquarian_request`` then
    this is the PK of the Fragment and the URL PK is for the Antiquarian;
    in this case all the ``FragmentLinks`` between this Antiquarian and
    this Fragment will be removed.

    If the form has no ``antiquarian_request`` then the URL PK is for the
    ``FragmentLink`` and this will be removed.
    """
    ...
    def get_object(self):
        """Get the object that we need to check the lock for: this is the Fragment."""
        return self.fragment
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if antiquarian_request is None:
            self.link = FragmentLink.objects.get(pk=pk)
            self.fragment = self.link.fragment
            self.antiquarian = self.link.antiquarian
        else:
            self.link = None
            self.fragment = Fragment.objects.get(pk=antiquarian_request)
    ...
    def get_success_url(self):
        return self.request.META.get(
            "HTTP_REFERER",
            reverse("fragment:detail", kwargs={"pk": self.fragment.pk}),
        )
    ...
    def delete(self, request, *args, **kwargs):
        ...
        how_many_links = FragmentLink.objects.filter(
            antiquarian=self.antiquarian,
            fragment=self.fragment,
        ).count()
        if how_many_links == 1:
